...
/*   Updated: 2017/05/30 03:46:11 by piquerue         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_open_menu_config(t_coucou *coucou)
{
	...
	{
		pt[0].x = 0;
		pt[1].x = coucou->win->width;
		pt[0].y = (i + 1) * split;
		pt[1].y = (i + 2) * split;
		if (coucou->texturepack == i)
			ft_mlx_draw_linept(pt[0], pt[1], coucou->img, create_color(0x0, 0x0, 0xFF));
		else
			ft_mlx_draw_linept(pt[0], pt[1], coucou->img, create_color(0xFF, 0xFF, 0xFF));
		i++;
	}
	(void)coucou;
}

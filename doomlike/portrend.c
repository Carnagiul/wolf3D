                                 ...
                                 float x2,float y2, float x3,float y3)
{
    return IntersectBox(x0,y0,x1,y1, x2,y2,x3,y3)
        && fabs(PointSide(x2,y2, x0,y0,x1,y1) + PointSide(x3,y3, x0,y0,x1,y1)) != 2
        && fabs(PointSide(x0,y0, x2,y2,x3,y3) + PointSide(x1,y1, x2,y2,x3,y3)) != 2;
}
...
#define Scaler_Init(a,b,c,d,f) \
    { d + (b-1 - a) * (f-d) / (c-a), ((f<d) ^ (c<a)) ? -1 : 1, \
      fabs(f-d), abs(c-a), (int)((b-1-a) * fabs(f-d)) % abs(c-a) }
...
static void GetSectorBoundingBox(int sectorno, struct xy* bounding_min, struct xy* bounding_max)
{
    ...
    for(int s = 0; s < (int)sect->npoints; ++s)
    {
...
rescan:;
    ...
    for(int s = 0; s < (int)sect->npoints; ++s)
    {
        ...
        float y = origin.y + ((fabs(target.x-origin.x) > fabs(target.z-origin.z))
        ...
        u = (fabs(dx) > fabs(dy) ? (unsigned)((x - vx1) * 1024 / dx)
                                 ...
                                 struct xyz point_in_wall, unsigned sectorno)
{
    ...
    for(unsigned qq=0; qq<nrandomvectors; ++qq)
    {
        ...
        if(IntersectRay(source, sectorno, target, -1 /* no particular sector */, &i) == 1) // hit something
        {
            ...
            float power = fabs(cosine) / (1.f + powf(len / fade_distance_radiosity, 2.0f));
